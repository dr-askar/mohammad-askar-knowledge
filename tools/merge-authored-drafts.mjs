...
const draft=JSON.parse(await readFile(join(root,'content/ophthalmology/drafts/uveitis.json'),'utf8'));
for(const chapter of draft.chapters){
  ...
  if(!matched.length)throw new Error(`${chapter.id}: keine vorhandene Gruppe für Quellen gefunden`);
  ...
  mapping.groups.push({id:chapter.id,titleDe:chapter.titleDe,category:'uveitis',status:'medical-review',primary,merged:refs.filter(ref=>ref!==primary),sourceLanguages:['de'],bodyDe:chapterBody(chapter),summaryDe:chapter.summaryDe,learningObjectives:chapter.learningObjectives,professional:chapter.professional,questions:chapter.questions,patient:chapter.patient});
}
await writeFile(mappingPath,JSON.stringify(mapping,null,2)+'\n');
console.log(`Merged ${draft.chapters.length} authored chapters into topic mapping.`);
function chapterBody(chapter){const p=chapter.professional;const list=items=>items.map(x=>`- ${x}`).join('\n');return `# ${chapter.titleDe}\n\n## Zusammenfassung\n\n${chapter.summaryDe}\n\n## Lernziele\n\n${list(chapter.learningObjectives)}\n\n## Fachärztlicher Überblick\n\n${p.overview.join('\n\n')}\n\n## Diagnostik\n\n${list(p.diagnostics)}\n\n## Differenzialdiagnosen\n\n${list(p.differentials)}\n\n## Therapieprinzipien\n\n${list(p.therapy)}\n\n## Red Flags\n\n${list(p.redFlags)}`}

...
function pageStart(title,description,depth='.'){
  return `<!doctype html><html lang="de" dir="ltr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="${esc(description)}"><meta name="robots" content="${publish?'index,follow':'noindex,nofollow'}"><title>${esc(title)} | Mohammad Askar</title><link rel="stylesheet" href="${depth}/assets/library.css?v=20260915"><link rel="stylesheet" href="${depth}/assets/lecture.css"></head><body><div class="kb-shell"><nav class="kb-nav" aria-label="Navigation"><a href="${depth}/index.html">← Augenheilkunde-Wissen</a><div class="kb-nav-links"><a href="${depth}/../../index.html#knowledge">Hauptseite</a><a href="${depth}/index.html">Alle Kapitel</a></div></nav>`
}
function renderIndex(items,topics){
  ...
  const cards=[...items.map(article=>({kind:'article',category:article.category,title:article.title,summary:article.summary,status:article.status,minutes:article.readingMinutes,url:`./${article.slug}/`})),...topics.map(topic=>({kind:'topic',category:topic.category,title:topic.title,summary:topic.summary,status:topic.status,minutes:Math.max(1,Math.ceil(topic.wordCount/250)),reports:topic.reportCount,url:`./topics/${topic.id}/`}))].map(item=>`<a class="kb-card" data-category="${esc(item.category)}" href="${esc(item.url)}"><span class="kb-badge">${categoryLabel(item.category)}</span><h2>${esc(item.title)}</h2><p>${esc(item.summary)}</p><footer><span>${item.minutes} Min.${item.reports?` · ${item.reports} Quellen`:''}</span><span class="kb-status">${statusLabel(item.status)}</span></footer></a>`).join('\n');
...
function publicRecord(article){return {slug:article.slug,title:article.title,category:article.category,language:'de',audiences:publish?['professional']:['professional','patient'],summary:article.summary,status:article.status,lastReviewed:article.lastReviewed,readingMinutes:article.readingMinutes,sourceCount:article.sourceKeys.length,url:`./${article.slug}/`}}

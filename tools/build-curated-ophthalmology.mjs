...
await writeFile(join(outDir,'original-source-links.json'),JSON.stringify({generatedAt:audit.generatedAt,reports:mapping.groups.map(group=>{const report=eligibleByPath.get(normalizeRef(group.primary));return {topicId:group.id,report:group.primary,sourceIds:report?.sourceIds||[],originalSources:report?.originalSources||[]}})},null,2)+'\n');
...
async function listEligibleReports(root){
  ...
  for(const notebook of notebooks){
    const studio=join(root,notebook.name,'Studio');let entries=[];const originalSources=await loadOriginalSources(join(root,notebook.name));
    ...
    for(const entry of entries.filter(entry=>entry.isFile()&&entry.name.toLowerCase().endsWith('.md')).sort((a,b)=>a.name.localeCompare(b.name))){
      ...
      const sourceIds=Array.isArray(metadata.source_ids)?metadata.source_ids:[];
      reports.push({absolute,relative:toPosix(relative(root,absolute)),notebook:notebook.name,file:entry.name,notebookId:metadata.notebook_id||'',artifactId:metadata.artifact_id||'',sourceIds,originalSources:sourceIds.map(id=>originalSources.get(id)).filter(Boolean)});
    }
  }
  return reports;
}
async function loadOriginalSources(notebookRoot){
  const manifest=join(notebookRoot,'Quellen','manifest.json');let data;try{data=JSON.parse(await readFile(manifest,'utf8'))}catch{return new Map()}
  return new Map((data.sources||[]).filter(source=>source.source_id).map(source=>[source.source_id,{title:source.title||'Originalquelle',type:source.type||'source',url:safeOriginalUrl(source.canonical_url)}]));
}
...
function markdownToHtml(markdown){
  ...
  for(let index=0;index<lines.length;index++){
    ...
    const quote=line.match(/^>\s*(.+)$/);if(quote){flush();const content=quote[1].replace(/^#{1,5}\s+/,'');const lower=content.toLocaleLowerCase('de');const isWarning=/red flag|warn|achtung|notfall|alarm/.test(lower);const className=isWarning?'kb-warning':'kb-takeaway';html+=`<aside class="${className}">${isWarning?`<strong>${inline(content)}</strong>`:inline(content)}</aside>`;continue}
...
function renderIndex(items){
  ...
  return `${pageStart('Augenheilkunde-Vorlesungen','Einzelne Lernkapitel aus den NotebookLM-Studio-Berichten.','../assets')}<nav class="kb-nav"><a href="../index.html">← Augenheilkunde-Wissen</a></nav><header class="kb-hero"><p class="kb-eyebrow">Lokale Prüffassung</p><h1>Einzelvorlesungen</h1><p>Jede Seite behandelt einen Studio-Bericht. Originalquellen sind verlinkt, soweit ihre URL im Quellenexport nachweisbar ist.</p><div class="kb-warning"><strong>Nicht veröffentlicht:</strong> Alle Inhalte befinden sich in medizinischer Prüfung.</div></header><main><div class="kb-controls"><label><span class="kb-eyebrow">Vorlesungen durchsuchen</span><input class="kb-search" type="search" placeholder="Vorlesung suchen" aria-label="Vorlesungen durchsuchen"></label><div class="kb-filters"><button class="kb-filter" data-filter="all" aria-pressed="true">Alle</button>${filters}</div></div><section class="kb-grid">${cards}</section><p class="kb-empty" hidden>Keine passenden Vorlesungen gefunden.</p></main><footer class="kb-footer">${items.length} Einzelvorlesungen · medizinische Prüfung ausstehend</footer></div><script src="../assets/library.js?v=20260915"></script></body></html>`;
}
function renderArticle(group,record,reports,body){
  ...
  const sources=reports.map(report=>{const originals=report.originalSources.filter(source=>source.url);const links=originals.map(source=>`<li><a href="${esc(source.url)}" target="_blank" rel="noopener noreferrer">${esc(source.title)}</a> <small>(${esc(source.type==='youtube'?'YouTube':source.type)})</small></li>`).join('');return `<li><strong>${esc(basename(report.file,'.md'))}</strong><br><small>${esc(report.notebook)}</small>${links?`<ul>${links}</ul>`:'<p>Original-URL im Quellenexport nicht nachweisbar.</p>'}</li>`}).join('');
